...
POST JSON to `/api/v1/invoices` with `customerName`, `customerEmail`, and `items` (`description`, `quantity`, `unitPrice`). The endpoint returns `202 Accepted`; poll `/api/v1/jobs/{id}`. Swagger is at `/swagger-ui.html`, health at `/actuator/health`, and Prometheus metrics at `/actuator/prometheus`.
